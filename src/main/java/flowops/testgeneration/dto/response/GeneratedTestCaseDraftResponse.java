...
public record GeneratedTestCaseDraftResponse(
        @Schema(description = "초안 ID", example = "1001")
        Long id,
        @Schema(description = "생성 작업 ID", example = "77")
        Long generationId,
        @Schema(description = "API ID", example = "10")
        Long apiId,
        @Schema(description = "초안 제목", example = "Generated happy path for API 10")
        String title,
        @Schema(description = "초안 설명", example = "Mocked AI-generated draft based on the current API metadata.")
        String description,
        @Schema(description = "초안 타입", example = "HAPPY_PATH")
        String type,
        @Schema(description = "사용자 역할", example = "QA_ENGINEER")
        String userRole,
        @Schema(description = "상태 조건", example = "Seed data available")
        String stateCondition,
        @Schema(description = "데이터 변형", example = "baseline")
        String dataVariant,
        @Schema(description = "요청 명세", example = "{\"body\":\"sample request\"}")
        String requestSpec,
        @Schema(description = "기대 명세", example = "{\"status\":200}")
        String expectedSpec,
        @Schema(description = "검증 명세", example = "{\"assertions\":[\"status == 200\"]}")
        String assertionSpec,
        @Schema(description = "중복 여부", example = "false")
        boolean duplicate,
        @Schema(description = "저장 선택 여부", example = "false")
        boolean selectedForSave,
        @Schema(description = "생성 일시", example = "2026-04-12T02:10:00")
